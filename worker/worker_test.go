...
func TestMain(m *testing.M) {
	...
	impl := logger.NewWithStandardLogger(l)
	logger.SetLogger(impl)
...
func TestReporter_Report(t *testing.T) {
	...
	r := &reporter{}
	r.Report(context.Background(), &Stats{QueueSize: 1234})

	s := buf.String()
	if !strings.Contains(s, "1234") {
		t.Errorf("Expected stats value is not contained: %s", s)
	}
}
